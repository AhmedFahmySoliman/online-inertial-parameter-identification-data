...
<head>
  <meta charset="UTF-8">
  <title>Code Capsule: Inertial Parameter Identification Experiments</title>
...
The figures from the corresponding paper are listed in the second column of $\mathbf{Table~2}$.  
They were generated using $\mathbf{MATLAB~R2024b}$.  <br>
<br>
...
<br>
The `main()` function accepts up to three input arguments $(i_1, i_2, i_3)$, which are defined separately for each figure.  
Refer to the fourth column of $\mathbf{Table 2}$ for the required input arguments to reproduce each figure.  
...
<tr><td>8</td><td>Computation time $t_c$ for two models walking with different velocities over different simulation runs.</td><td>1S</td><td>$main(i_1,i_2)$ <br> $i_1 = '1S'$ <br> $i_2 \in \{1,2,\dots,12\}^2 \subset \mathbb{Z}^2$</td></tr>
<tr><td>9</td><td>Relative error $e_{rel}$ for four models walking with different velocities over different simulation runs.</td><td>2S</td><td>$main(i_1,i_2)$ <br> $i_1 = '2S'$ <br> $i_2 \in \{1,2,\dots,12\}^4 \subset \mathbb{Z}^4$</td></tr>
